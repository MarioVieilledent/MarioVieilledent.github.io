export const translations = {
  title: [
    "Travelling Rat",
    "Rat Voyageur",
    "Ratto Viaggiatore",
    "Rata Viajera",
    "Reisende Rotte",
    "Seyahat Eden Sıçan",
    "Ταξιδευτικός Αρουραίος",
    "旅するネズミ",
    "旅行的老鼠",
    "Potujoča podgana",
    "الجرذ المسافر",
  ],
  websiteDescription: [
    "Mario Vieilledent - My personal website: open source maps, dev, recipes and learning Norwegian",
    "Mario Vieilledent - Mon site perso : cartes open source, dev, recettes et apprentissage du norvégien",
    "Mario Vieilledent - Il mio sito personale: mappe open source, sviluppo, ricette e apprendimento del norvegese",
    "Mario Vieilledent - Mi sitio personal: mapas de código abierto, desarrollo, recetas y aprendizaje del noruego",
    "Mario Vieilledent - Min personlige nettside: åpen kildekode-kart, utvikling, oppskrifter og norsklæring",
    "Mario Vieilledent - Kişisel web sitem: açık kaynak haritalar, yazılım geliştirme, tarifler ve Norveççe öğrenimi",
    "Mario Vieilledent - Η προσωπική μου ιστοσελίδα: χάρτες ανοιχτού κώδικα, προγραμματισμός, συνταγές και εκμάθηση νορβηγικών",
    "マリオ・ヴィエイユダン - 私の個人ウェブサイト：オープンソースの地図、開発、レシピ、そしてノルウェー語の学習",
    "Mario Vieilledent - 我的个人网站：开源地图、开发、食谱和挪威语学习",
    "Mario Vieilledent - Moja osebna spletna stran: odprtokodni zemljevidi, razvoj, recepti in učenje norveščine",
    "Mario Vieilledent - موقعي الشخصي: خرائط مفتوحة المصدر، وتطوير البرمجيات، ووصفات، وتعلّم اللغة النرويجية",
  ],
  language: [
    "Language",
    "Langue",
    "Lingua",
    "Idioma",
    "Språk",
    "Dil",
    "Γλώσσα",
    "言語",
    "语言",
    "Jezik",
    "اللغة",
  ],
  dragAndDropFilesHere: [
    "Drag and drop files here",
    "Glissez-déposez des fichiers ici",
    "Trascina e rilascia i file qui",
    "Arrastra y suelta archivos aquí",
    "Dra og slipp filer hit",
    "Dosyaları buraya sürükleyip bırakın",
    "Σύρετε και αποθέστε αρχεία εδώ",
    "ここにファイルをドラッグ＆ドロップ",
    "将文件拖放到此处",
    "Datoteke povlecite in spustite sem",
    "اسحب الملفات وأفلتها هنا",
  ],
  baseMap: [
    "Base map",
    "Fond de carte",
    "Mappa di base",
    "Mapa base",
    "Basiskart",
    "Temel harita",
    "Βασικός χάρτης",
    "ベースマップ",
    "底图",
    "Osnovni zemljevid",
    "الخريطة الأساسية",
  ],
  layers: [
    "Layers",
    "Couches",
    "Livelli",
    "Capas",
    "Lag",
    "Katmanlar",
    "Επίπεδα",
    "レイヤー",
    "图层",
    "Plasti",
    "الطبقات",
  ],
  points: [
    "Points",
    "Points",
    "Punti",
    "Puntos",
    "Punkter",
    "Noktalar",
    "Σημεία",
    "ポイント",
    "地点",
    "Točke",
    "النقاط",
  ],
  shops: [
    "Shops",
    "Magasins",
    "Negozi",
    "Tiendas",
    "Butikker",
    "Mağazalar",
    "Καταστήματα",
    "店舗",
    "商店",
    "Trgovine",
    "المتاجر",
  ],
  services: [
    "Services",
    "Services",
    "Servizi",
    "Servicios",
    "Tjenester",
    "Hizmetler",
    "Υπηρεσίες",
    "サービス",
    "服务",
    "Storitve",
    "الخدمات",
  ],
  outdoorSites: [
    "Outdoor",
    "Plein air",
    "Natura",
    "Aire libre",
    "Friluftsliv",
    "Açık hava",
    "Υπαίθρια",
    "アウトドア",
    "户外",
    "Na prostem",
    "أماكن خارجية",
  ],
  sportSites: [
    "Sports sites",
    "Sites sportifs",
    "Impianti sportivi",
    "Instalaciones deportivas",
    "Idrettsanlegg",
    "Spor tesisleri",
    "Αθλητικές εγκαταστάσεις",
    "スポーツ施設",
    "体育设施",
    "Športni objekti",
    "المرافق الرياضية",
  ],
  clearLayers: [
    "Clear layers",
    "Effacer les couches",
    "Cancella i livelli",
    "Borrar capas",
    "Klare lag",
    "Katmanları temizle",
    "Εκκαθάριση επιπέδων",
    "レイヤーをクリア",
    "清除图层",
    "Počisti plasti",
    "مسح الطبقات",
  ],
  notAvailableIn3D: [
    "Not available in 3D view",
    "Non disponible en vue 3D",
    "Non disponibile in vista 3D",
    "No disponible en vista 3D",
    "Ikke tilgjengelig i 3D-visning",
    "3D görünümde kullanılamaz",
    "Μη διαθέσιμο σε προβολή 3D",
    "3Dビューでは利用できません",
    "3D视图中不可用",
    "Ni na voljo v 3D-pogledu",
    "غير متاح في العرض ثلاثي الأبعاد",
  ],
  general: [
    "General",
    "Général",
    "Generale",
    "General",
    "General",
    "Genel",
    "Γενικά",
    "一般",
    "常规",
    "Splošno",
    "عام",
  ],
  topographic: [
    "Topographic",
    "Topographique",
    "Topografico",
    "Topográfico",
    "Topografisk",
    "Topografik",
    "Τοπογραφικός",
    "地形",
    "地形",
    "Topografsko",
    "طبوغرافية",
  ],
  imagery: [
    "Satellite imagery",
    "Imagerie satellite",
    "Immagini satellitari",
    "Imágenes de satélite",
    "Satellittbilder",
    "Uydu görüntüleri",
    "Δορυφορική απεικόνιση",
    "衛星画像",
    "影像卫星图像",
    "Satelitski posnetki",
    "صور الأقمار الصناعية",
  ],
  hybrid: [
    "Hybrid",
    "Hybride",
    "Ibrido",
    "Híbrido",
    "Hybrid",
    "Hibrit",
    "Υβριδικός",
    "ハイブリッド",
    "混合",
    "Hibridno",
    "هجينة",
  ],
  transport: [
    "Transport",
    "Transport",
    "Trasporti",
    "Transporte",
    "Transport",
    "Ulaşım",
    "Μεταφορές",
    "交通",
    "交通",
    "Prevoz",
    "النقل",
  ],
  other: [
    "Other",
    "Autre",
    "Altro",
    "Otro",
    "Andre",
    "Diğer",
    "Άλλα",
    "その他",
    "其他",
    "Drugo",
    "أخرى",
  ],
  "2dMercator": [
    "2D Mercator",
    "Mercator 2D",
    "Mercatore 2D",
    "Mercator 2D",
    "Mercator 2D",
    "2D Merkator",
    "2D Μερκατορικός",
    "2D メルカトル",
    "2D 墨卡托",
    "2D Mercator",
    "مركاتور ثنائي الأبعاد",
  ],
  "3dGlobe": [
    "3D Globe",
    "Globe 3D",
    "Globo 3D",
    "Globo 3D",
    "Globe 3D",
    "3D Küre",
    "3D Υδρόγειος",
    "3D グローブ",
    "3D 地球",
    "3D globus",
    "كرة أرضية ثلاثية الأبعاد",
  ],
  sport: [
    "Sport",
    "Sport",
    "Sport",
    "Deporte",
    "Sport",
    "Spor",
    "Αθλητισμός",
    "スポーツ",
    "运动",
    "Šport",
    "الرياضة",
  ],
  train: [
    "Train",
    "Train",
    "Treno",
    "Entrenamiento",
    "Tog",
    "Tren",
    "Τρένο",
    "電車",
    "火车",
    "Vlak",
    "قطار",
  ],
  loading: [
    "Loading",
    "Chargement",
    "Caricamento",
    "Cargando",
    "Chargement",
    "Yükleniyor",
    "Φόρτωση",
    "読み込み中",
    "加载中",
    "Nalaganje",
    "جارٍ التحميل",
  ],
  translateThisWord: [
    "Translate this word",
    "Traduis ce mot",
    "Traduci questa parola",
    "Traduce esta palabra",
    "Oversett dette ordet",
    "Bu kelimeyi çevir",
    "Μετάφρασε αυτή τη λέξη",
    "この単語を翻訳",
    "翻译这个词",
    "Prevedi to besedo",
    "ترجم هذه الكلمة",
  ],
  english: [
    "English",
    "Anglais",
    "Inglese",
    "Inglés",
    "Engelsk",
    "İngilizce",
    "Αγγλικά",
    "英語",
    "英语",
    "Angleščina",
    "الإنجليزية",
  ],
  norwegian: [
    "Norwegian",
    "Norvégien",
    "Norvegese",
    "Noruego",
    "Norsk",
    "Norveççe",
    "Νορβηγικά",
    "ノルウェー語",
    "挪威语",
    "Norveščina",
    "النرويجية",
  ],
  learnNorwegian: [
    "Learn Norwegian",
    "Apprendre le Norvégien",
    "Impara il norvegese",
    "Aprender noruego",
    "Lær norsk",
    "Norveççe öğrenin",
    "Μάθε νορβηγικά",
    "ノルウェー語を学ぶ",
    "学习挪威语",
    "Uči se norveščino",
    "تعلّم النرويجية",
  ],
  turkishFlashcards: [
    "Turkish Flashcards",
    "Cartes mémoire Turc",
    "Flashcard di turco",
    "Tarjetas de vocabulario de turco",
    "Tyrkiske glosekort",
    "Türkçe Flaş Kartlar",
    "Κάρτες εκμάθησης Τουρκικών",
    "トルコ語の単語カード",
    "土耳其语闪卡",
    "Turške flash kartice",
    "بطاقات تعليمية تركية",
  ],
  typeYourTranslation: [
    "Type your translation",
    "Tapez votre traduction",
    "Scrivi la tua traduzione",
    "Escribe tu traducción",
    "Skriv inn oversettelsen din",
    "Çevirinizi yazın",
    "Πληκτρολόγησε τη μετάφρασή σου",
    "翻訳を入力",
    "输入您的翻译",
    "Vpišite svoj prevod",
    "اكتب ترجمتك",
  ],
  next: [
    "Next",
    "Suivant",
    "Avanti",
    "Siguiente",
    "Neste",
    "Sonraki",
    "Επόμενο",
    "次へ",
    "下一个",
    "Naprej",
    "التالي",
  ],
  submit: [
    "Submit",
    "Valider",
    "Invia",
    "Validar",
    "Valider",
    "Gönder",
    "Υποβολή",
    "送信",
    "提交",
    "Potrdi",
    "إرسال",
  ],
  iDontKnow: [
    "I don't know",
    "Je ne sais pas",
    "Non lo so",
    "No lo sé",
    "Jeg vet ikke",
    "Bilmiyorum",
    "Δεν ξέρω",
    "わかりません",
    "我不知道",
    "Ne vem",
    "لا أعرف",
  ],
  home: [
    "Home",
    "Accueil",
    "Home",
    "Inicio",
    "Hjem",
    "Ana Sayfa",
    "Αρχική",
    "ホーム",
    "首页",
    "Domov",
    "الرئيسية",
  ],
  back: [
    "Back",
    "Retour",
    "Indietro",
    "Volver",
    "Tilbake",
    "Geri",
    "Πίσω",
    "戻る",
    "返回",
    "Nazaj",
    "رجوع",
  ],
  pageNotFound: [
    "404 - Wrong address :D Nothing here ",
    "404 - Circulez, y'a rien à voir.",
    "404 - Pagina non trovata",
    "404 - Página no encontrada",
    "404 - Siden finnes ikke",
    "404 - Sayfa bulunamadı",
    "404 - Λάθος διεύθυνση :D Δεν υπάρχει τίποτα εδώ",
    "404 - ページが見つかりません",
    "404 - 页面未找到",
    "404 - Napačen naslov :D Tu ni ničesar",
    "404 - عنوان خاطئ :D لا يوجد شيء هنا ",
  ],
  words: [
    "words",
    "mots",
    "parole",
    "palabras",
    "ord",
    "kelimeler",
    "λέξεις",
    "単語数",
    "单词",
    "besede",
    "كلمات",
  ],
  success: [
    "correct",
    "correct",
    "corretto",
    "correcto",
    "korrekt",
    "doğru",
    "σωστό",
    "正解",
    "正确",
    "pravilno",
    "صحيح",
  ],
  failure: [
    "wrong",
    "faux",
    "sbagliato",
    "incorrecto",
    "falsk",
    "yanlış",
    "λάθος",
    "不正解",
    "错误",
    "napačno",
    "خطأ",
  ],
  statistics: [
    "Statistics",
    "Statistiques",
    "Statistiche",
    "Estadísticas",
    "Statistikk",
    "İstatistikler",
    "Στατιστικά",
    "統計",
    "统计",
    "Statistika",
    "الإحصاءات",
  ],
  listOfWords: [
    "List of words",
    "Liste des mots",
    "Elenco di parole",
    "Lista de palabras",
    "Liste over ord",
    "Kelime listesi",
    "Λίστα λέξεων",
    "単語一覧",
    "单词列表",
    "Seznam besed",
    "قائمة الكلمات",
  ],
  searchHere: [
    "Search a word (en, no)",
    "Chercher un mot (en, no)",
    "Cerca una parola (en, no)",
    "Buscar una palabra (en, no)",
    "Slå opp et ord (en, no)",
    "Bir kelime arayın (en, no)",
    "Αναζήτηση λέξης (en, no)",
    "単語を検索（英語・ノルウェー語）",
    "搜索单词（英、挪）",
    "Išči besedo (en, no)",
    "ابحث عن كلمة (بالإنجليزية أو النرويجية)",
  ],
  note: [
    "Note",
    "Note",
    "Nota",
    "Nota",
    "Notat",
    "Not",
    "Σημείωση",
    "メモ",
    "备注",
    "Opomba",
    "ملاحظة",
  ],
  recipes: [
    "Recipes",
    "Recettes",
    "Ricette",
    "Recetas",
    "Oppskrifter",
    "Tarifler",
    "Συνταγές",
    "レシピ",
    "菜谱",
    "Recepti",
    "الوصفات",
  ],
  feasts: [
    "Feasts",
    "Festins",
    "Feste",
    "Banquetes",
    "Fester",
    "Ziyafetler",
    "Γιορτές",
    "ごちそう",
    "盛宴",
    "Gozbe",
    "الولائم",
  ],
  starters: [
    "Starters",
    "Entrées",
    "Antipasti",
    "Entradas",
    "Forretter",
    "Başlangıçlar",
    "Ορεκτικά",
    "前菜",
    "前菜",
    "Predjedi",
    "المقبلات",
  ],
  mainCourses: [
    "Main Courses",
    "Plats",
    "Piatti principali",
    "Platos principales",
    "Hovedretter",
    "Ana Yemekler",
    "Κυρίως πιάτα",
    "主菜",
    "主菜",
    "Glavne jedi",
    "الأطباق الرئيسية",
  ],
  desserts: [
    "Desserts",
    "Desserts",
    "Dolci",
    "Postres",
    "Desserter",
    "Tatlılar",
    "Επιδόρπια",
    "デザート",
    "甜点",
    "Sladice",
    "الحلويات",
  ],
  breads: [
    "Breads",
    "Pains",
    "Pane",
    "Panes",
    "Brød",
    "Ekmekler",
    "Ψωμιά",
    "パン",
    "面包",
    "Kruhi",
    "الخبز",
  ],
  dips: [
    "Dips",
    "Dips",
    "Salse",
    "Salsas",
    "Dipper",
    "Soslar",
    "Ντιπ",
    "ディップ",
    "蘸酱",
    "Pomake",
    "صلصات التغميس",
  ],
  recipesPageDescription1: [
    "This page brings together my recipes and the shared meals that I call feasts.",
    "Cette page regroupe mes recettes et les repas partagés que j’appelle festins.",
    "Questa pagina raccoglie le mie ricette e i pasti condivisi che chiamo banchetti.",
    "Esta página reúne mis recetas y las comidas compartidas que llamo festines.",
    "Denne siden samler oppskriftene mine og måltidene jeg deler, som jeg kaller festmåltider.",
    "Bu sayfa tariflerimi ve şölen adını verdiğim paylaşılan yemekleri bir araya getiriyor.",
    "Αυτή η σελίδα συγκεντρώνει τις συνταγές μου και τα κοινά γεύματα που αποκαλώ γιορτές.",
    "このページでは、私のレシピと「ごちそう」と呼んでいる共有の食事をまとめています。",
    "此页面汇集了我的食谱以及我称之为盛宴的共享餐食。",
    "Ta stran združuje moje recepte in skupne obroke, ki jih imenujem gozbe.",
    "تجمع هذه الصفحة وصفاتي والوجبات المشتركة التي أسمّيها ولائم.",
  ],
  recipesPageDescription2: [
    "I love garlic, but I hate cheese.",
    "J'adore l'ail mais j'ai horreur du fromage.",
    "Adoro l'aglio, ma odio il formaggio.",
    "Me encanta el ajo, pero odio el queso.",
    "Jeg elsker hvitløk, men jeg hater ost.",
    "Sarımsağı çok seviyorum ama peynirden nefret ediyorum.",
    "Λατρεύω το σκόρδο, αλλά μισώ το τυρί.",
    "私はニンニクが大好きですが、チーズが大嫌いです。",
    "我非常喜欢大蒜，但我讨厌奶酪。",
    "Obožujem česen, vendar sovražim sir.",
    "أحب الثوم، لكنني أكره الجبن.",
  ],
  recipesPageDescription3: [
    "A great lover of Turkish cuisine 🇹🇷, I consider it to be among the top 5 greatest cuisines in the world.",
    "Grand amateur de cuisine turque 🇹🇷, je la considère comme faisant partie du top 5 des plus grandes cuisines du monde.",
    "Grande amante della cucina turca 🇹🇷, la considero tra le prime 5 cucine più grandi del mondo.",
    "Gran amante de la cocina turca 🇹🇷, la considero entre las 5 mejores cocinas del mundo.",
    "Som en stor tilhenger av tyrkisk mat 🇹🇷, anser jeg den som en av de 5 beste kjøkkenene i verden.",
    "Türk mutfağının 🇹🇷 büyük bir hayranı olarak, onu dünyanın en iyi 5 mutfağından biri olarak görüyorum.",
    "Μεγάλος λάτρης της τουρκικής κουζίνας 🇹🇷, τη θεωρώ μία από τις 5 κορυφαίες κουζίνες στον κόσμο.",
    "トルコ料理 🇹🇷 の大ファンであり、世界で最も偉大な料理トップ5の一つだと考えています。",
    "作为土耳其菜 🇹🇷 的忠实爱好者，我认为它位列世界五大顶级菜系之一。",
    "Sem velik ljubitelj turške kuhinje 🇹🇷 in menim, da sodi med 5 najboljših kuhinj na svetu.",
    "أنا من عشاق المطبخ التركي 🇹🇷، وأعتبره من بين أفضل خمسة مطابخ في العالم.",
  ],
  ingredients: [
    "Ingredients",
    "Ingrédients",
    "Ingredienti",
    "Ingredientes",
    "Ingredienser",
    "Malzemeler",
    "Υλικά",
    "材料",
    "食材",
    "Sestavine",
    "المكونات",
  ],
  instructions: [
    "Instructions",
    "Instructions",
    "Istruzioni",
    "Instrucciones",
    "Instruksjoner",
    "Talimatlar",
    "Οδηγίες",
    "手順",
    "步骤说明",
    "Navodila",
    "التعليمات",
  ],
  notes: [
    "Notes",
    "Notes",
    "Note",
    "Notas",
    "Notater",
    "Notlar",
    "Σημειώσεις",
    "備考",
    "小贴士",
    "Opombe",
    "ملاحظات",
  ],
  idea: [
    "Idea",
    "Idée",
    "Idea",
    "Idea",
    "Idé",
    "Fikir",
    "Ιδέα",
    "アイデア",
    "创意",
    "Ideja",
    "فكرة",
  ],
  menu: [
    "Menu",
    "Menu",
    "Menu",
    "Menú",
    "Meny",
    "Menü",
    "Μενού",
    "メニュー",
    "菜单",
    "Meni",
    "القائمة",
  ],
  pictures: [
    "Pictures",
    "Photos",
    "Foto",
    "Fotos",
    "Photos",
    "Fotoğraflar",
    "Εικόνες",
    "写真",
    "照片",
    "Slike",
    "الصور",
  ],
  flags: [
    "Flags",
    "Drapeaux",
    "Bandiere",
    "Banderas",
    "Drapeaux",
    "Bayraklar",
    "Σημαίες",
    "旗",
    "旗帜",
    "Zastave",
    "الأعلام",
  ],
  browseCategories: [
    "Browse",
    "Parcourir",
    "Sfoglia",
    "Explorar",
    "Bla gjennom",
    "Göz at",
    "Περιήγηση",
    "閲覧する",
    "浏览",
    "Prebrskaj",
    "تصفح",
  ],
  randomRecipe: [
    "Random recipe",
    "Recette aléatoire",
    "Ricetta casuale",
    "Receta aleatoria",
    "Tilfeldig oppskrift",
    "Rastgele tarif",
    "Τυχαία συνταγή",
    "ランダムレシピ",
    "随机食谱",
    "Naključni recept",
    "وصفة عشوائية",
  ],
  recipesNumber: [
    "Number of recipes:",
    "Nombre de recettes :",
    "Numero di ricette:",
    "Número de recetas:",
    "Antall oppskrifter:",
    "Tarif sayısı:",
    "Αριθμός συνταγών:",
    "レシピの数：",
    "食谱数量：",
    "Število receptov:",
    "عدد الوصفات:",
  ],
  recipesWithCheese: [
    "Number of recipes with cheese:",
    "Nombre de recettes avec du fromage :",
    "Numero di ricette con formaggio:",
    "Número de recetas con queso:",
    "Antall oppskrifter med ost:",
    "Peynirli tarif sayısı:",
    "Αριθμός συνταγών με τυρί:",
    "チーズを使ったレシピの数：",
    "含奶酪的食谱数量：",
    "Število receptov s sirom:",
    "عدد الوصفات التي تحتوي على الجبن:",
  ],
  search: [
    "Search",
    "Rechercher",
    "Cerca",
    "Buscar",
    "Søk",
    "Ara",
    "Αναζήτηση",
    "検索",
    "搜索",
    "Iskanje",
    "بحث",
  ],
  clearFilters: [
    "Clear filters",
    "Effacer les filtres",
    "Cancella filtri",
    "Borrar filtros",
    "Fjern filtre",
    "Filtreleri temizle",
    "Εκκαθάριση φίλτρων",
    "フィルターをクリア",
    "清除筛选",
    "Počisti filtre",
    "مسح عوامل التصفية",
  ],
  restultsFound: [
    "results found",
    "résultats trouvés",
    "risultati trovati",
    "resultados encontrados",
    "resultater funnet",
    "sonuç bulundu",
    "αποτελέσματα βρέθηκαν",
    "件の結果が見つかりました",
    "个结果",
    "najdenih rezultatov",
    "نتيجة موجودة",
  ],
  allFeastsJSON: [
    "All feasts (JSON)",
    "Tous les festins (JSON)",
    "Tutte le feste (JSON)",
    "Todas las banquetes (JSON)",
    "Alle fester (JSON)",
    "Tüm ziyafetler (JSON)",
    "Όλες οι γιορτές (JSON)",
    "すべての祭り(まつり)（JSON）",
    "所有节日 (JSON)",
    "Vse gozbe (JSON)",
    "جميع الولائم (JSON)",
  ],
  allRecipesJSON: [
    "All recipes (JSON)",
    "Toutes les recettes (JSON)",
    "Tutte le ricette (JSON)",
    "Todas las recetas (JSON)",
    "Alle oppskrifter (JSON)",
    "Tüm tarifler (JSON)",
    "Όλες οι συνταγές (JSON)",
    "すべての料理(りょうり)（JSON）",
    "所有食谱 (JSON)",
    "Vsi recepti (JSON)",
    "جميع الوصفات (JSON)",
  ],
  export: [
    "Export",
    "Exporter",
    "Esporta",
    "Exportar",
    "Eksporter",
    "Dışa aktar",
    "Εξαγωγή",
    "エクスポート",
    "导出",
    "Izvozi",
    "تصدير",
  ],
  picturesOfMyDishes: [
    "Pictures of my dishes",
    "Photos de mes plats",
    "Foto dei miei piatti",
    "Fotos de mis platos",
    "Bilder av rettene mine",
    "Yemeklerimin fotoğrafları",
    "Φωτογραφίες των πιάτων μου",
    "自分の料理の写真",
    "我的菜肴照片",
    "Slike mojih jedi",
    "صور أطباقي",
  ],
  translationCoverage: [
    "Translation coverage",
    "Couverture des traductions",
    "Copertura delle traduzioni",
    "Cobertura de traducciones",
    "Oversettelsesdekning",
    "Çeviri kapsamı",
    "Κάλυψη μεταφράσεων",
    "翻訳の対応状況",
    "翻译覆盖情况",
    "Pokritost prevodov",
    "تغطية الترجمة",
  ],
  translationCoverageDescription: [
    "Select a cell to open that item in the corresponding language.",
    "Sélectionnez une cellule pour ouvrir cet élément dans la langue correspondante.",
    "Seleziona una cella per aprire l'elemento nella lingua corrispondente.",
    "Selecciona una celda para abrir ese elemento en el idioma correspondiente.",
    "Velg en celle for å åpne elementet på det tilhørende språket.",
    "İlgili öğeyi karşılık gelen dilde açmak için bir hücre seçin.",
    "Επιλέξτε ένα κελί για να ανοίξετε αυτό το στοιχείο στην αντίστοιχη γλώσσα.",
    "セルを選択すると、該当する言語でその項目を開きます。",
    "选择一个单元格，以使用相应语言打开该项目。",
    "Izberite celico, da odprete element v ustreznem jeziku.",
    "حدد خلية لفتح هذا العنصر باللغة المقابلة.",
  ],
  feastOrRecipe: [
    "Feast or recipe",
    "Festin ou recette",
    "Banchetto o ricetta",
    "Banquete o receta",
    "Festmåltid eller oppskrift",
    "Ziyafet veya tarif",
    "Γιορτή ή συνταγή",
    "ごちそうまたはレシピ",
    "宴席或食谱",
    "Pojedina ali recept",
    "وليمة أو وصفة",
  ],
  noRanking: [
    "No ranking",
    "Aucun classement",
    "Nessuna classifica",
    "Sin clasificación",
    "Ingen rangering",
    "Sıralama yok",
    "Χωρίς κατάταξη",
    "ランキングなし",
    "无排名",
    "Brez uvrstitve",
    "بلا تصنيف",
  ],
};
